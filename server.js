...
async function handleUploadImage(req, res) {
  try {
    ...
    const profileImage = buildStoredImageDataUrl(fileName, fileContentBase64);
    const updatedUser = await usersCollection.findOneAndUpdate(
      { email: email.value },
      {
        $set: {
...
function buildStoredImageDataUrl(fileName, fileContentBase64) {
  const extension = path.extname(fileName).toLowerCase();
  const contentTypes = {
    ".png": "image/png",
    ".jpg": "image/jpeg",
  ...
  return `data:${contentType};base64,${String(fileContentBase64 || "").trim()}`;
}
